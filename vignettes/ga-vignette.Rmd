...
ga("chr2", 127156000, geneLocs) # Get candidate gene probabilities
...
ga("chr2", 127156000, geneLocs, lambda=1.0e-5, maxDist=2e6, minPP=0) # Get candidate gene probabilities
```
...
The minimum required input for `ga()` is a chromosome identifier in the same format than in the gene models and a chromosomal position in the same build than the gene models. The `ga()` function has additional optional input:
...
Because both files are supplied as examples, they are limited to chromosome 2.
